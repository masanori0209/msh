...
| L5 高度 | 代入既定値 | `: ${z:=persist}; echo $z` | ✅ |
| L5 高度 | プロセス置換 <( ) | `cat <(echo proc)` | ✅ |
...
- **L3**: スクリプト言語（目標 100%）
- **L4**: 高度な移行体験（目標 80%+）
- **L5**: 高度構文（連想配列・`<( )` 入力置換は対応、`>( )` は bash 委譲）

## 既知の制約

- 連想配列の複合初期化 `declare -A m=([k]=v)`（個別代入 `m[k]=v` は対応）
- パラメータ展開: `${var:-}` / `${var:=}` / `${var:+}` / `${var:?}` / `${var#}` / `${var%}` / `${var//}` / `${var^^}` / `${var:off:len}` 対応
- 算術は四則・剰余・括弧・変数のみ（`**`・三項・ビット演算は未対応）
- プロセス置換 `>( )` のみ未対応（`<( )` はネイティブ、出力側は bash 委譲）
