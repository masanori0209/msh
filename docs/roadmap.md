...
- [x] **B-1**（v0.7.3）: 構造化出力 `msh --json -c`（command/exit_code/duration_ms/stdout/stderr を 1 行 JSON）
- [ ] B-2: エージェント安全実行 `msh --agent`（破壊的操作の確認ゲート・構造化ログ）
- [ ] B-3: MCP サーバ公開（Claude/Cursor から安全に shell を駆動）
...
| **v1.0.0** | Phase 6 | 安定版、WASM プラグイン、エコシステム整備 |
...
- [ ] 添字の算術評価 `${arr[i+1]}`
- [ ] `${var:=default}` の永続代入（値は返すが変数へは未代入）
- [ ] `$SECONDS` / `$LINENO`
- [ ] プロセス置換 `<( )` / `>( )`
...
1. **互換(L5)** — パラメータ展開の高度形 `${var:-}`/`${var//}`、`$RANDOM`/`$SECONDS`
2. **配布** — Homebrew tap・GitHub Releases バイナリ（CI でクロスビルド）
3. **拡張** — WASM プラグイン ABI 確定＋PoC
4. **信頼** — 実物 dotfiles コーパスの互換回帰を CI に常設
5. **ベンチ** — 交互計測を `benchmark.sh` に固定化、対話モード RSS の継続計測
